...
/**
 * A {@code Session} is a stateful data context associated with a single Subject
 * (user, daemon process, etc) who interacts with a software system over a
 * period of time.
 * <p/>
 * A {@code Session} is intended to be managed by the business tier and
 * accessible via other tiers without being tied to any given client technology.
 * This is a <em>great</em> benefit to Java systems, since until now, the only
 * viable session mechanisms were the {@code javax.servlet.http.HttpSession} or
 * Stateful Session EJB's, which many times unnecessarily coupled applications
 * to web or ejb technologies.
 *
 * @since 0.1
 */
public interface Session {
	 ...
	 * creation.
	 * <p/>
	 * All return values from this method are expected to have proper
	 * {@code toString()}, {@code equals()}, and {@code hashCode()}
	 * implementations. Good candidates for such an identifier are
	 * {@link java.util.UUID UUID}s, {@link Integer Integer}s, and {@link String
	 * String}s.
	 *
	 ...
	 * </ul>
	 * <b>*Note:</b> if you are used to the {@code HttpSession}'s
	 * {@code getMaxInactiveInterval()} method, the scale on this method is
	 * different: Shiro Sessions use millisecond values for timeout whereas
	 * {@code HttpSession.getMaxInactiveInterval} uses seconds. Always use
	 * millisecond values with Shiro sessions.
	 *
	 * @return the time in milliseconds the session may remain idle before
	 *         expiring.
	 * @throws InvalidSessionException
	 *             if the session has been stopped or expired prior to calling
	 *             this method.
	 * @since 0.2
	 */
	 ...
	 * <p/>
	 * Most programmers won't use this method directly and will instead rely on
	 * the last access time to be updated automatically as a result of an
	 * incoming web request or remote procedure call/method invocation.
	 * <p/>
	 ...
	 * enough period of time, the user's server-side session could time-out.
	 * Again, such cases are rare since most rich-clients frequently require
	 * server-side method invocations.
	 * <p/>
	 * In this example though, the user's session might still be considered
	 * valid because the user is actively &quot;using&quot; the application,
	 * just not communicating with the server. But because no server-side method
	 * calls are invoked, there is no way for the server to know if the user is
	 * sitting idle or not, so it must assume so to maintain session integrity.
	 * This {@code touch()} method could be invoked by the rich-client
	 * application code during those times to ensure that the next time a
	 * server-side method is invoked, the invocation will not throw an
	 * {@link ExpiredSessionException ExpiredSessionException}. In short terms,
	 * it could be used periodically to ensure a session does not time out.
	 * <p/>
	 * How often this rich-client &quot;maintenance&quot; might occur is
	 * entirely dependent upon the application and would be based on variables
	 * such as session timeout configuration, usage characteristics of the
	 * client application, network utilization and application server
	 * performance.
	 *
	 * @throws InvalidSessionException
	 *             if this session has stopped or expired prior to calling this
	 *             method.
	 */
	 ...
	 * resources.
	 * <p/>
	 * If this session has already been authenticated (i.e. the {@code Subject}
	 * that owns this session has logged-in), calling this method explicitly
	 * might have undesired side effects:
	 * <p/>
	 * It is common for a {@code Subject} implementation to retain
	 * authentication state in the {@code Session}. If the session is explicitly
	 * stopped by application code by calling this method directly, it could
	 * clear out any authentication state that might exist, thereby effectively
	 * &quot;unauthenticating&quot; the {@code Subject}.
	 * <p/>
	 * As such, you might consider
	 * {@link org.apache.shiro.subject.Subject#logout logging-out} the 'owning'
	 * {@code Subject} instead of manually calling this method, as a log out is
	 * expected to stop the corresponding session automatically, and also allows
	 * framework code to execute additional cleanup logic.
	 *
	 * @throws InvalidSessionException
	 *             if this session has stopped or expired prior to calling this
	 *             method.
	 ...
	 *
	 * @param key
	 *            the name under which the {@code value} object will be bound in
	 *            this session
	 * @param value
