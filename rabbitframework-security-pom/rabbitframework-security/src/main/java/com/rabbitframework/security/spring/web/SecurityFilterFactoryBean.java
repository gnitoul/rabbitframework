 ...
 * {@link javax.servlet.Filter Filter} beans defined independently in your
 * Spring application context. Upon discovery, they will be automatically added
 * to the {@link #setFilters(java.util.Map) map} keyed by the bean ID. That ID
 * can then be used in the filter chain definitions, for example:
 *
 * <pre>
 ...
 * to all configured Filter instances so you don't have to specify them
 * individually on each filter instance. To ensure your own custom filters
